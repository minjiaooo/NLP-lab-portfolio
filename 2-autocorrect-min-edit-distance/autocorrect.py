...
Original file is located at
    https://colab.research.google.com/drive/1C1AoVO4ehiz_JAOhZmytGLfb7IROwxYX
"""
...
def probable_substitutes(word, probs, maxret = 10):
    ...
    def edit_two_letters(word):
        word_set1 = edit_one_letter(word)
        word_set2 = set()
        for w in word_set1:
            word_set2.update(edit_one_letter(w))
        return set(word_set2)

    candidates = edit_two_letters(word)

    valid_candidates = []
    for w in candidates:
      if w in probs and w != word:
        valid_candidates.append(w)

    #sorting
    sorted_candidates = sorted(valid_candidates, key=lambda w: probs[w], reverse=True)

    if len(sorted_candidates) < maxret:
      maxret = len(sorted_candidates)
    res = []
    for w in sorted_candidates[:maxret]:
      res.append((w, probs[w]))
...
def min_edit_distance(source, target, ins_cost = 1,
    ...
    #caculate other edit distance
    for i in range(1,m+1):
      for j in range(1,n+1):
        if source[i-1] == target[j-1]:
          curr_replace_cost = 0
        else:
          curr_replace_cost = rep_cost

        D[i,j] = min(
            D[i-1,j] + del_cost,
            D[i,j-1]+ ins_cost,
            D[i-1,j-1] + curr_replace_cost
        )
    med = D[m,n]
